...
class DeviceManageFormBase extends FFSForm{
    ...
    public function InitEditPanel($objDevice = null){
        ...
        $this->AddWidget(
            ((is_null($objDevice))?'Create Device':'Edit Device'),
            'icon-edit',
            $this->pnlEdit
        );
    }
    public function InitList($arrDevices){
        ...
        $this->lstDevices->AddAction(
            new MJaxTableEditInitEvent(),
            new MJaxServerControlAction($this, 'lstDevice_editInit')
        );
        $this->lstDevices->AddAction(
            new MJaxTableEditSaveEvent(),
            new MJaxServerControlAction($this, 'lstDevice_editSave')
        );
        $this->AddWidget(
            'Devices',
            'icon-ul',
