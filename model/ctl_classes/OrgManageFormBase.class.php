...
class OrgManageFormBase extends FFSForm{
    ...
    public function InitEditPanel($objOrg = null){
        ...
        $this->AddWidget(
            ((is_null($objOrg))?'Create Org':'Edit Org'),
            'icon-edit',
            $this->pnlEdit
        );
    }
    public function InitList($arrOrgs){
        ...
        $this->lstOrgs->AddAction(
            new MJaxTableEditInitEvent(),
            new MJaxServerControlAction($this, 'lstOrg_editInit')
        );
        $this->lstOrgs->AddAction(
            new MJaxTableEditSaveEvent(),
            new MJaxServerControlAction($this, 'lstOrg_editSave')
        );
        $this->AddWidget(
            'Orgs',
            'icon-ul',
